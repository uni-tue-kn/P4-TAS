control StreamGate(inout header_t hdr, 
    ...
    // Used for GateClosedDueToOctetsExceeded
    RegisterAction<bit<32>, bit<12>, bit<1>>(first_sdu_per_interval) get_first_sdu = {
        void apply(inout bit<32> first_sdu, out bit<1> read_value){
    ...
    RegisterAction<bit<32>, bit<12>, void>(octets_per_interval) reset_octets = {
        void apply(inout bit<32> remaining_octets) {
            ...
            remaining_octets = ig_md.stream_gate.max_octets_interval;
        }
    };
    ...
    action set_gate_and_ipv(bit<1> gate_state, bit<4> ipv, bit<12> interval_identifier, bit<32> max_octets_interval) {
                            
        ig_md.stream_gate.PSFPGateEnabled = gate_state;
        ig_md.stream_gate.ipv = ipv;
        ig_md.stream_gate.max_octets_interval = max_octets_interval;
        ig_md.stream_gate.interval_identifier = interval_identifier;

        stream_gate_counter.count();
    }

    action missed_time_slice(){
        stream_gate_counter.count();
            ...
            if (stream_gate_instance.apply().miss){
                ...
                if (ig_md.stream_gate.gate_closed_due_to_octets_exceeded_enable){
                    ...
                    if (ig_md.stream_gate.reset_octets == true){
                        ...
                        ig_md.to_be_dropped = get_first_sdu.execute(ig_md.stream_gate.interval_identifier);
                    }
                }
                ...
                if (ig_md.stream_gate.ipv == 8){
                    ...
                    ig_tm_md.qid = (bit<7>)hdr.eth_802_1q.pcp;
                } else {
                    ig_tm_md.qid = (bit<7>)ig_md.stream_gate.ipv;
                }
            }
        }
    }
